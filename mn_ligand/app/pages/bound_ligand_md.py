...
from mn_ligand.app.components.molstar_viewer import (
    ChainVisualization,
...
def _trajectory_to_multimodel_pdb(
    ...
    from mn_ligand.ligandx.services.md.workflow.trajectory_processor import TrajectoryProcessorRunner
...
$3Dmolpromise.then(function() {{
  var v = {viewer_var};
  if (!v) return;
  var storageKey = "mnlig_view_{safe_key}";
  try {{
...
<div class="mnlig-frame-badge">{esc}</div>
"""
    return badge + html
...
def _ligand_pdb_to_mol2(ligand_pdb: str) -> str | None:
    ...
    if not obabel:
        return None
    with tempfile.TemporaryDirectory(prefix="mnlig_mol2_") as tmpdir:
...
def _render_run_construction(
    ...
    with st.expander("Ligand-X parameters exposed by MDOptimizationConfig"):
        st.caption("Source: mn_ligand/ligandx/services/md/config.py")
        st.table(
            [
