...
class SolvatedSystemBuilder:
    ...
    LIGAND_RMSD_FAIL_A = float(os.getenv("MN_LIGAND_ASSEMBLY_RMSD_FAIL_A", "0.05"))
    LIGAND_LOCK_K_KJMOL_NM2 = float(os.getenv("MN_LIGAND_LOCK_K_KJMOL_NM2", "2500.0"))
    LIGAND_PLANARITY_K_KJMOL_NM2 = float(os.getenv("MN_LIGAND_PLANARITY_K_KJMOL_NM2", "1500.0"))
    ...
    def __init__(self, output_dir: str = "data/md_outputs"):
        ...
        self.enable_ligand_restraints = str(
            os.getenv("MN_LIGAND_ENABLE_LIGAND_RESTRAINTS", "1")
        ).strip().lower() not in {"0", "false", "no", "off"}
        self.enable_protein_restraints = str(
            os.getenv("MN_LIGAND_ENABLE_PROTEIN_RESTRAINTS", "1")
        ).strip().lower() not in {"0", "false", "no", "off"}
        self.protein_restraint_selection = str(
            os.getenv("MN_LIGAND_PROTEIN_RESTRAINT_SELECTION", "backbone")
        ).strip().lower()
