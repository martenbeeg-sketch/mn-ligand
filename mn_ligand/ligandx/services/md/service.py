...
class MDOptimizationService:
    ...
    def _lazy_init(self):
        ...
        from mn_ligand.ligandx.lib.chemistry import get_pdb_parser, get_component_analyzer, get_protein_preparer
    ...
    def _create_system_from_amber_artifacts(self, config: MDOptimizationConfig) -> Dict[str, Any]:
        ...
        def _resolve(path_value: Optional[str]) -> Optional[str]:
            ...
            marker = "mn-ligand/"
            if marker in p:
                mapped = os.path.join("/mn-ligand", p.split(marker, 1)[1])
