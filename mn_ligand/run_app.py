...
def main() -> None:
    st.set_page_config(page_title="mn-ligand", layout="wide")
    ...
    st.sidebar.divider()
    default_runs = Path(__file__).resolve().parents[1] / "mn-ligand-workdir" / "workdir" / "runs"
    st.sidebar.caption(f"Runs: {os.getenv('MN_LIGAND_RUN_DIR', str(default_runs))}")
    pg.run()
